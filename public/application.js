...
/***/ function(module, exports, __webpack_require__) {
	...
	var poop = __webpack_require__(12);
	var gameUtil = __webpack_require__(15);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var onChangePassword = function onChangePassword(event) {
	  event.preventDefault();
	  var data = getFormFields(event.target);
	  api.passwordChange(data).then(ui.changePasswordSuccess).catch(ui.passwordChangeFailure);
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var haventSubmittedEnd = { over: true };
	...
	var onShowGame = function onShowGame(event) {
	  ...
	  if (gameId.length === 0) {
	    api.showGames().then(ui.showGames).catch(ui.onShowError);
	  } else {
	    api.showGame(gameId).then(ui.showGame).catch(ui.onShowError);
	  }
	};
	...
	var onJoinGame = function onJoinGame(event) {
	  event.preventDefault();
	  var data = getFormFields(event.target);
	  api.patch(data).then(ui.onPatchSuccess).catch(ui.onError);
	};

	var onLogGame = function onLogGame(event) {
	  event.preventDefault();
	  var data = getFormFields(event.target);
	  api.patch(data).then(ui.onPatchSuccess).catch(ui.onError);
	};
	...
	var onUpdateGame = function onUpdateGame(event) {
	  if (haventSubmittedEnd.over) {
	    event.preventDefault();
	    api.updateGame(+$(this).data('index'), $(this).text(), index.gameOver.state).then(ui.onPatchSuccess).catch(ui.onError);
	...
	module.exports = {
	  ...
	  haventSubmittedEnd: haventSubmittedEnd,
	  resetFun: resetFun
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ function(module, exports, __webpack_require__) {
	...
	var updateGame = function updateGame(indx, currentPlayer, state) {
	  return $.ajax({
	...
	var joinGame = function joinGame(data, id) {
	  return $.ajax({
	    url: config.apiOrigin + '/games/' + id,
	    headers: {
	      Authorization: 'Token token=' + store.user.token
	...
	var logGame = function logGame(data, id) {
	  return $.ajax({
	    url: config.apiOrigin + '/games/' + id,
	    method: 'PATCH',
	    headers: {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var showGame = function showGame(data) {
	  $('.log').text('Board state of game ' + $('#game-id').val() + ' was ' + data.game.cells[0] + ' | ' + data.game.cells[1] + ' | ' + data.game.cells[2] + ' | ' + data.game.cells[3] + ' | ' + data.game.cells[4] + ' | ' + data.game.cells[5] + ' | ' + data.game.cells[6] + ' | ' + data.game.cells[7] + ' | ' + data.game.cells[8]);
	};

	var showSuccess = function showSuccess() {};

	var onShowError = function onShowError() {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var whichPlayer = { player: 'X' };
	var count = 0;
	var gameOver = { state: false };

	var winCondition = function winCondition(player, indexOne, indexTwo, indexThree) {
	  return indexOne === player && indexTwo === player && indexThree === player;
	};

	var checkHorizontalWin = function checkHorizontalWin(player) {
	  return winCondition(player, gameBoard[0], gameBoard[1], gameBoard[2]) || winCondition(player, gameBoard[3], gameBoard[4], gameBoard[5]) || winCondition(player, gameBoard[6], gameBoard[7], gameBoard[8]);
	};

	var checkDiagonalWin = function checkDiagonalWin(player) {
	  return winCondition(player, gameBoard[0], gameBoard[4], gameBoard[8]) || winCondition(player, gameBoard[2], gameBoard[4], gameBoard[6]);
	};

	var checkVerticalWin = function checkVerticalWin(player) {
	  return winCondition(player, gameBoard[0], gameBoard[3], gameBoard[6]) || winCondition(player, gameBoard[1], gameBoard[4], gameBoard[7]) || winCondition(player, gameBoard[2], gameBoard[5], gameBoard[8]);
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = __webpack_require__.p + "4aa578ed562a112eb216416d426e6682.jpg";

/***/ },
/* 25 */
/***/ function(module, exports, __webpack_require__) {

	module.exports = __webpack_require__.p + "e8b325764365a066c4520d34bc109490.jpg";
